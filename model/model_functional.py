...
from tensorflow.keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, Lambda
...
def DarknetConv2D_BN_Leaky(inputs, n_filters, kernel_size=(3, 3), down_sample=False, bn_act=True, layer_idx=None):
    ...
    x = Conv2D(n_filters, kernel_size=kernel_size, strides=strides, padding=padding, use_bias=use_bias,
               kernel_initializer=RandomNormal(stddev=0.02), kernel_regularizer=l2(5e-4))(inputs)
    if bn_act:
...
def UpSampleConv(inputs, n_filters, layer_idx=0):
    ...
    if isinstance(inputs, (list, tuple)):
        # Handle case when inputs is a list of tensors
        x = DarknetConv2D_BN_Leaky(inputs[0], n_filters, kernel_size=(1, 1), layer_idx=layer_idx)
        x = UpSampling2D(2)(x)
        x = Concatenate(axis=-1)([x, inputs[1]])
        idx = 3
    
    x = DarknetConv2D_BN_Leaky(x, n_filters, kernel_size=(1, 1), layer_idx=layer_idx + idx)
    x = DarknetConv2D_BN_Leaky(x, n_filters * 2, layer_idx=layer_idx + idx + 1)
    x = DarknetConv2D_BN_Leaky(x, n_filters, kernel_size=(1, 1), layer_idx=layer_idx + idx + 2)
    x = DarknetConv2D_BN_Leaky(x, n_filters * 2, layer_idx=layer_idx + idx + 3)
    x = DarknetConv2D_BN_Leaky(x, n_filters, kernel_size=(1, 1), layer_idx=layer_idx + idx + 4)

    return x


def ScalePrediction(inputs, n_filters, num_classes, layer_idx=0):
    x = DarknetConv2D_BN_Leaky(inputs, n_filters, layer_idx=layer_idx)
